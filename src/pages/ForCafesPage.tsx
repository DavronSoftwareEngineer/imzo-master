    ...
    <>
      ...
      <section className="section-sm bg-white">
        <div className="contain text-center" style={{ maxWidth: 720, margin:'0 auto' }}>
          <p style={{ color:'#64748b', fontSize:16, lineHeight:1.7 }}>{t('cafe.description')}</p>
        </div>
      </section>
          ...
          <section key={sysId} className={`section${i % 2 === 0 ? ' bg-surface' : ' bg-white'}`}>
            <div className="contain">
              <div className="grid-2" style={{ alignItems:'start' }}>
                <div className={`rounded-2xl overflow-hidden shadow-lg bg-gradient-to-br ${gradients[i]} flex items-center justify-center`} style={{ minHeight: 360 }}>
                  <span className="text-7xl py-20">{emojis[i]}</span>
                </div>
                <div>
                  <h2 style={{ fontSize:24, fontWeight:800, color:'#0f172a', marginBottom:16 }}>{t(`cafe.systems.${sysId}.name`)}</h2>
                  <p style={{ color:'#64748b', marginBottom:24, lineHeight:1.7 }}>{t(`cafe.systems.${sysId}.description`)}</p>
                  <h3 style={{ fontSize:18, fontWeight:700, color:'#0f172a', marginBottom:16 }}>{t('cafe.features')}</h3>
